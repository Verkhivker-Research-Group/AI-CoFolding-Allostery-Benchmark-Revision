# 2_run_models/collect_af3_outputs.py
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--af3-root", required=True)
...
def score_hint_from_name(path: Path) -> float | None:
    ...
    m = re.search(r"(?:rank[_-]?)(\d+)", name)
    if m:
        return float(m.group(1))

    m = re.search(r"(?:model[_-]?)(\d+)", name)
    if m:
        return float(m.group(1))

    m = re.search(r"(?:score[_-]?)(\d+(?:\.\d+)?)", name)
    if m:
        return float(m.group(1))

    m = re.search(r"(?:plddt|ptm)[_-]?(\d+(?:\.\d+)?)", name)
    if m:
        return -float(m.group(1))
